...
function MovieDetails() {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `https://api.themoviedb.org/3/${media_type.toLowerCase()}/${movie_id}?api_key=${API_KEY}`
      )
  ...
  return (
    <div className="min-h-screen bg-zinc-900 text-white">
      ...
      <div className="relative">
        <img
          src={
            imgBase(movieDetails.backdrop_path, "w1280") ||
            imgBase(movieDetails.poster_path, "w1280")
          }
          alt={title}
      ...
      <div className="container mx-auto px-4 md:px-8 -mt-24 md:-mt-32 relative">
        <div className="flex flex-col md:flex-row gap-6">
          ...
          <div className="flex-none">
            <img
              src={
                imgBase(movieDetails.poster_path) ||
                imgBase(movieDetails.backdrop_path)
              }
              alt={title}
          ...
          <div className="flex-1">
            ...
            <div className="flex flex-wrap items-center gap-3 mt-4">
              <div className="flex items-center space-x-2">
                <span className="inline-block bg-amber-400 text-zinc-900 font-semibold px-3 py-1 rounded">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {movieDetails.genres?.map((g) => (
                <span
                  key={g.id}
                  className="text-sm px-2 py-1 bg-zinc-700/60 rounded"
                >
                  {g.name}
                </span>
              ))}
            ...
            <section className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-zinc-300">
              <div className="space-y-2">
                <div>
                  <strong className="text-zinc-200">Budget:</strong>{" "}
                  <span className="text-zinc-300">
                    {formatCurrency(movieDetails.budget)}
                  </span>
                </div>
                <div>
                  <strong className="text-zinc-200">Revenue:</strong>{" "}
                  <span className="text-zinc-300">
                    {formatCurrency(movieDetails.revenue)}
                  </span>
                </div>
                <div>
                  <strong className="text-zinc-200">Original language:</strong>{" "}
                  <span className="text-zinc-300">
                    {(movieDetails.original_language || "").toUpperCase()}
                  </span>
                </div>
                <div>
                  <strong className="text-zinc-200">Popularity:</strong>{" "}
                  <span className="text-zinc-300">
              ...
              <div className="space-y-2">
                <div>
                  <strong className="text-zinc-200">Spoken languages:</strong>{" "}
                  <span className="text-zinc-300">
                ...
                <div>
                  <strong className="text-zinc-200">Country:</strong>{" "}
                  <span className="text-zinc-300">
                ...
                <div className="flex gap-3 mt-2">
                  {movieDetails.homepage && (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="px-3 py-2 bg-emerald-500 text-black rounded text-sm"
                    >
                      Official Site
                    </a>
                  )}
                  {movieDetails.imdb_id && (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="px-3 py-2 bg-yellow-400 text-black rounded text-sm"
                    >
                      IMDb
                    </a>
                  )}
